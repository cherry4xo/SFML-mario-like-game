...
bool TileMap::load(const std::string& tmx_file_path)
{
	...
	const size_t spacing = std::stoul(tileset->Attribute("spacing"));
	const size_t margin = std::stoul(tileset->Attribute("margin"));
	...
	for (size_t y = 0u; y < rows; ++y)
		for (size_t x = 0u; x < columns; ++x)
			texture_grid.emplace_back(sf::Vector2f((float)x * (tile_width + margin), (float)y * (tile_height + margin)));
		...
		for (size_t y = 0u, index = 0u; y < height; ++y)
		{
			for (size_t x = 0u; x < width; ++x, ++index)
			{
				...
				{   // ќбходим по часовой стрелке
					...
					leftTop.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + margin, texture_grid[tile_num - 1].y + margin);
					rightTop.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + tile_width - margin, texture_grid[tile_num - 1].y + margin);
					rightBottom.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + tile_width - margin, texture_grid[tile_num - 1].y + tile_height - margin);
					leftBottom.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + margin, texture_grid[tile_num - 1].y + tile_height - margin);
